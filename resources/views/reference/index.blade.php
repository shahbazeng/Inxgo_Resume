    ...
    <div class="row justify-content-center">
        <div class="col-md-9">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <h3 class="text-info" style="display:inline-block">Your Reference Summery</h3>
                    <div class="text-right" style="display:inline-block;float:right">
                        <a href="{{route('references_create')}}" class="btn btn-outline-success">Add More</a>
                    </div>
                    <hr>
                          ...
                          @endphp
                           @foreach ($data as $d)
                            <tr>
                                <th scope="row">{{ $i++}}</th>
                                <td>{{ $d->name}}</td>
                                <td>{{ $d->year}}-{{ $d->year_end}}</td>
                                <td>
                                    <a href="{{ url('references_update/'.base64_encode($d->id)) }}" class="btn btn-outline-primary btn-sm">Edit</a>
                                    <a href="{{ url('references_delete/'.base64_encode($d->id)) }}" class="btn btn-outline-danger btn-sm" onclick="return confirm('Are you sure ?')">Delete</a>
                            ...
                            <tr>
                             ...
                             <tr>
                                <td colspan="5">
                                    <a href="{{route('skills_index')}}" class="btn btn-block btn-primary">Back to Skills</a>
                                </td>
                            </tr>
                        </tbody>
                    </table>
